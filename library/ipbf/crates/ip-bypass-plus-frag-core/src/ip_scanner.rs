...
fn compute_score(entry: &IpProbeEntry, config: &crate::config::Config) -> u8 {
    // Balanced scoring: speed-focused but fair
    let upload_pts = match entry.upload_bps {
        None => 0.0,
        Some(bps) => 20.0 * (bps / config.UPLOAD_SPEED_CAP_BPS).min(1.0),
    };
    ...
    let tcp_pts = match entry.tcp_latency_ms {
        None => 0.0,
        Some(ms) => 15.0 * (1.0 - (ms as f64 / config.TCP_LATENCY_CAP_MS).min(1.0)),
    };
    ...
    let ttfb_pts = match entry.ttfb_ms {
        None => 0.0,
        Some(ms) => 15.0 * (1.0 - (ms as f64 / config.TTFB_CAP_MS).min(1.0)),
    };
    let tls_flat = if entry.tls_ok { 5.0f64 } else { 0.0 };
    ...
    (upload_pts + download_pts + tcp_pts + tls_pts + ttfb_pts + tls_flat + cert_pts + bonus)
        .round() as u8
}
...
/// Parse CIDR ranges from ip_list file, returning raw range strings and their host counts.
pub fn parse_cidr_ranges(path: impl AsRef<std::path::Path>) -> Vec<(String, usize)> {
    ...
    for line in text.lines() {
        ...
        if let Ok(net) = line.parse::<IpNet>() {
            let count = match &net {
                IpNet::V4(v4net) => v4net.hosts().count(),
                IpNet::V6(v6net) => v6net.hosts().count(),
            };
            ranges.push((line.to_string(), count));
        }
    }
    ranges
}
